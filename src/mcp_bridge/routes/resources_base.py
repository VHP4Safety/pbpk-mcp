"""Shared read-only MCP resource endpoints."""
...
class SchemaDocumentResource(CamelModel):
    ...
    example_document: Optional[dict[str, Any]] = Field(default=None, alias="example")
...
def _contract_runtime_hint() -> str:
    return "Install the packaged contract artifacts or provide the runtime patch manifest copy."
...
async def list_simulation_resources(
    ...
    if last_modified_candidates:
        headers["Last-Modified"] = _isoformat(max(last_modified_candidates))
...
async def list_schema_resources(
    ...
    etag = _weak_etag(
        [item["_fingerprint"] for item in page_items] + [str(page), str(limit), search or ""]
    )
    headers = {"ETag": etag}
    if page_items:
